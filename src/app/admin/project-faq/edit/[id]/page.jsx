// ✅ 5. Edit Page: /src/app/admin/project-faq/edit/[id]/page.jsx
"use client";
import { useEffect, useState } from "react";
import { useParams, useRouter } from "next/navigation";

export default function EditProjectFaqPage() {
  const router = useRouter();
  const { id } = useParams();
  const [form, setForm] = useState({ question: "", answer: "", _id: "" });

  useEffect(() => {
    const fetchFaq = async () => {
      const res = await fetch("/api/project-faq");
      const data = await res.json();
      const faq = data.find((item) => item._id === id);
