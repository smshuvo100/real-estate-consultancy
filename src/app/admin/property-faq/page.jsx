...
export default function PropertyFaqAdminPage() {
  const [faqs, setFaqs] = useState([]);

  useEffect(() => {
    fetch("/api/property-faq")
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) => {
        console.error("❌ Failed to fetch property faqs:", err);
      });
  }, []);
  ...
  return (
    <div className="dashboard-content" style={{ padding: 20 }}>
      <div className="flex-between" style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: "24px" }}>❓ Property FAQ</h1>
        <Link href="/admin/property-faq/create" className="add-btn">
          ...
          faqs.map((faq) => (
            <div key={faq._id} className="blog-card">
              ...
              <div className="action-buttons">
                <Link
                  href={`/admin/property-faq/edit/${faq._id}`}
                  className="edit-btn"
                >
